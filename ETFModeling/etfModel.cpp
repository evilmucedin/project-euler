...
PriceData PriceData::subPriceData(const SizeTVector& indices) {
    PriceData result;
...
PriceData loadData(const StringVector& tickers) {
    ...
    set<string> dates;
    {
        ...
        for (const auto& ticker : tickers) {
            ...
            auto dividends = df->getColumn("Dividends", ticker, fileName);
...
StrategyResult minMaxStrategy(const PriceData& pd, size_t i) {
    ...
    ModelResult& res = result.buyAndHold;
    ModelResult& resMinMax = result.minMax;
