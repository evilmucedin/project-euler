...
string modeToString(ios_base::openmode mode) {
  static constexpr array<ios_base::openmode, 6> kModeValV{
      ios_base::in,  ios_base::out,   ios_base::app,
      ios_base::ate, ios_base::trunc, ios_base::binary};

  static const array<string, kModeValV.size()> kModeNameV{
...
void checkMode(const string& filename, ios_base::openmode mode) {
  if ((mode & ios_base::trunc) && !(mode & ios_base::out)) {
    throw Exception(string("strict_fstream: open('") + filename +
...
void checkOpen(ios* s_p, const string& filename, ios_base::openmode mode) {
  if (s_p->fail()) {
    throw Exception(string("strict_fstream: open('") + filename + "'," +
                    modeToString(mode) + "): open failed: " + strError());
  }
}
...
void checkPeek(istream* is_p, const string& filename, ios_base::openmode mode) {
  ...
  if (peekFailed) {
    throw Exception(string("strict_fstream: open('") + filename + "'," +
                    modeToString(mode) + "): peek failed: " + strError());
  }
  is_p->clear();
}
}

IFstream::IFstream(const string& filename, ios_base::openmode mode) {
  open(filename, mode);
}
...
OFstream::OFstream(const string& filename, ios_base::openmode mode) {
  open(filename, mode);
}
